...
select deptno, round(avg(sal),2) "급여 평균", round(stddev(sal), 2) "표준편차"
from emp
group by deptno
order by deptno;

-- Ex2. 직무별 직무, 직원수, 급여 최댓값, 최솟값, 평균을 직무 오름차순으로 출력.
select job, count(job) "직원수", max(sal) "최댓값", min(sal) "최솟값", round(avg(sal), 2) "평균"
from emp
group by job
order by job;
...
--          정렬 순서: (1) 부서번호 (2) 직무
select deptno, job, count(*) "직원수", round(avg(sal), 2) "급여 평균"
from emp
group by deptno, job
order by deptno, job;

-- Ex4. 입사연도별 사원수를 검색. (힌트) to_char(날짜, 포맷) 이용.
select to_char(hiredate,'YYYY') "YEAR", count(hiredate) "COUNT"
from emp
...
-- 부서별 급여 평균. 부서별 급여 평균이 2000 이상인 부서만 검색.
...
select deptno, count(*) "직원수", round(avg(sal), 0) "급여 평균"
from emp
...
from emp
where job != 'PRESIDENT' --having절에도 사용 가능
group by job
having count(job) >= 3
order by job;
...
-- 연도별 오름차순
select to_char(hiredate, 'YYYY') "입사연도", deptno, count(*) "사원수" 
from emp
